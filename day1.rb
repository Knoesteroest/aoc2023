...
class Calibration
    def self.testCalibrationExample
    ...
    nineninekfp49
        ...
        actual = get_calibration_value_sum(input)
        if 142 == actual 
            puts "testCalibrationExample works" 
        else
            puts "testCalibrationExample fails, returned #{actual}"
        end
    end

    def self.get_calibration_value_sum(input)
        #lines = breakIntoLines(input)
        values = getValues(input.lines(chomp: true))
        return values.sum
    end

    def self.breakIntoLines(text)
        result = text.split('+')
        return result
    end

    def self.testBreak
        actual = self.breakIntoLines("f4wfw+
        nuigherguhrek+nuhgfreuhgfkr")
        if 3 == actual.size
            puts "breakIntoLines works" 
        else
            puts "breakIntoLines fails, returned #{actual}"
        end
    end

    def self.getValues(lines)
        values = Array.new
        for line in lines
    ...
    def self.getValue(line)
        concat = getFirstDigit(line) + getLastDigit(line)
        return concat.to_i
    end

    def self.getFirstDigit(line)
        return line[/(\d)/]
    end

    def self.testGetFirstDigit
        input = "freuhfkeh3jhkjuh4kjbub6"
        actual = getFirstDigit(input)
        if '3' == actual 
            puts "testGetFirstDigit works" 
        else
            puts "testGetFirstDigit fails, returned #{actual}"
        end
    end

    def self.getLastDigit(line)
        return line[/(\d)(?:\D)*$/][0]
    end
end

#Calibration.testBreak
#Calibration.testGetFirstDigit
Calibration.testCalibrationExample